...
![image-20230322230631465](.\img\image-20230322230631465.png)
...
<img src=".\img\image-20230322231024190.png" alt="image-20230322231024190" style="zoom:50%;" />
...
+ ![image-20230322231509610](.\img\image-20230322231509610.png)
...
+ 第四步：在package.json中创建scripts脚本，执行脚本打包即可
  ...
  + <img src=".\img\image-20230322233012347.png" alt="image-20230322233012347" style="zoom: 67%;" />

### 1.6 webpack配置文件

在根目录下创建一个**webpack.config.js**（默认名字）文件，来作为webpack的配置文件：

<img src=".\img\image-20230323090706381.png" alt="image-20230323090706381" style="zoom:50%;" />
...
![image-20230323091032080](.\img\image-20230323091032080.png)
...
+ **内联方式**：内联方式使用较少，因为不方便管理；
  ...
  + ![image-20230323091841632](.\img\image-20230323091841632.png)
...
+ **loader配置方式**
  ...
  + <img src=".\img\image-20230323092824436.png" alt="image-20230323092824436" style="zoom:50%;" />
...
+ **配置**：
  ...
  + ![image-20230323093153275](.\img\image-20230323093153275.png)
...
+ 安装less-loader `npm install less-loader -D`
  ...
  + **配置webpack.config.js**
    ...
    + ![image-20230323093859917](.\img\image-20230323093859917.png)
...
+ 第二步：选择可以添加你需要的PostCSS相关的插件；
  ...
  + 转换之后的css
    + <img src=".\img\image-20230323094719180.png" alt="image-20230323094719180" style="zoom:50%;" />
...
+ **配置**（webpack.config.js中）： 因为postcss需要有对应的插件才会起效果，所以我们需要**配置它的plugin**；
  + <img src=".\img\image-20230323095211289.png" alt="image-20230323095211289" style="zoom:50%;" />

+ **单独配置**（postcss.config.js）:在根目录下创建postcss.config.js
  + ![image-20230323095631852](.\img\image-20230323095631852.png)
  ...
  + 安装postcss-preset-env：`npm install postcss-preset-env -D`
    + ![image-20230323100136088](.\img\image-20230323100136088.png)
    + **在使用某些postcss插件时，也可以直接传入字符串**
      + ![image-20230323100220110](.\img\image-20230323100220110.png)
...
+ **配置：** 
  + ![image-20230326131955611](.\img\image-20230326131955611.png)
  + **文件的命名规则：**
    + [ext]： 处理文件的扩展名；
    + [name]：处理文件的名称；
    ...
    + ![image-20230326132241373](.\img\image-20230326132241373.png)
    + **outputPath** 设置输出的文件夹。
...
+ **配置：** 
  + ![image-20230326132515065](.\img\image-20230326132515065.png)
  ...
  + **limit**属性：以限制哪些大小的图片转换和不转换
    + ![image-20230326132715512](.\img\image-20230326132715512.png)
...
+ **配置**
  + 自定义文件的输出路径和文件名
    + 方式一：修改output，添加assetModuleFilename属性；
      + ![image-20230326133249402](.\img\image-20230326133249402.png)
    + 方式二：在Rule中，添加一个generator属性，并且设置filename；
      + ![image-20230326133347443](.\img\image-20230326133347443.png)
  + url-loader的limit效果
    + 步骤一：将type修改为asset；
    + 步骤二：添加一个parser属性，并且制定dataUrl的条件，添加maxSize属性；
      + ![image-20230326133502915](.\img\image-20230326133502915.png)
  + **加载字体文件**
    + 可以选择使用file-loader来处理，也可以选择直接使用webpack5的资源模块类型来处理；
      + ![image-20230326133631349](.\img\image-20230326133631349.png)
...
+ **配置:**
  + ![image-20230326200845048](.\img\image-20230326200845048.png)
  + 我们必须添加@vue/compiler-sfc来对template进行解析：`npm install @vue/compiler-sfc -D`
  + 配置对应的Vue插件
    + ![image-20230326201012008](.\img\image-20230326201012008.png)
    + ![image-20230326201018488](.\img\image-20230326201018488.png)
...
+ **配置：** 
  + ![image-20230326133955592](.\img\image-20230326133955592.png)
...
+ **配置：** 
  + ![image-20230326134125704](.\img\image-20230326134125704.png)

+ **分析：**
  + 我们会发现，现在自动在dist文件夹中，生成了一个index.html的文件：该文件中也自动添加了我们打包的bundle.js文件；
    + ![image-20230326135335195](.\img\image-20230326135335195.png)
  + **文件的生成过程**
...
+ **自定义HTML模板**
  + 如果我们想在自己的模块中加入一些比较特别的内容：
    ...
    + 这个我们需要一个属于自己的index.html模块：
      + ![image-20230326135623474](.\img\image-20230326135623474.png)
      + 上面的代码中，会有一些**类似这样的语法<% 变量%>，这个是EJS模块**填充数据的方式。
  + **配置 HtmlWebpackPlugin时，添加如下配置：**
    + template：指定我们要使用的模块所在的路径；
    + title：在进行htmlWebpackPlugin.options.title读取时，就会读到该信息；
    + ![image-20230326140619690](.\img\image-20230326140619690.png)
...
+ ![image-20230326140924157](.\img\image-20230326140924157.png)
...
+ 区别：
  + ![image-20230326141501426](.\img\image-20230326141501426.png)

### 1.10 Babel

​	Babel是一个工具链，主要用于旧浏览器或者环境中将ECMAScript 2015+代码转换为向后兼容版本的JavaScript；包括：语法转换、源代码转换等；

![image-20230326190955203](.\img\image-20230326190955203.png)
...
+ **原理：**
  ...
  + ![image-20230326191814068](.\img\image-20230326191814068.png)
  + ![image-20230326191823980](.\img\image-20230326191823980.png)
...
+ **Babel-loader**
  ...
  + **必须指定插件才能生效**
    + ![image-20230326192140419](.\img\image-20230326192140419.png)
    + 如果我们一个个去安装使用插件，那么需要手动来管理大量的babel插件，我们可以直接给webpack提供一个preset，webpack会根据我们的预设来加载对应的插件列表，并且将其传递给babel。
      + 比如常见的预设有三个：env、react、Typescript
      + **安装：** `npm install @babel/preset-env`
      + ![image-20230326192311382](.\img\image-20230326192311382.png)
  ...
  + **Babel的配置文件**
    ...
    + ![image-20230326192602504](.\img\image-20230326192602504.png)
...
+ ![image-20230326195140238](.\img\image-20230326195140238.png)
...
+ **全局标识的配置**
  + ![image-20230326200621523](.\img\image-20230326200621523.png)
...
+ 开启方式：
  ...
  + 方式二：在启动webpack的命令种，添加 --watch的标识
    + ![image-20230416123452734](.\img\image-20230416123452734.png)
...
+ 修改配置文件，告知dev server，从什么位置查找文件：
  + <img src=".\img\image-20230416124140101.png" alt="image-20230416124140101" style="zoom: 50%;" /><img src=".\img\image-20230416124153631.png" alt="image-20230416124153631" style="zoom:50%;" />
  + <img src=".\img\image-20230416124213289.png" alt="image-20230416124213289" style="zoom:50%;" />
...
+ 模块热替换（HMR）
  ...
  + 开启HMR
    + 修改webpack的配置：
      + ![image-20230416124615911](.\img\image-20230416124615911.png)
    + 浏览器可以看到如下效果：
      + ![image-20230416124628609](.\img\image-20230416124628609.png)
    + 但当我们修改了某一个模块的代码时，依然是刷新的整个页面，们需要去指定哪些模块发生更新时，进行HMR；
      + ![image-20230416124712209](.\img\image-20230416124712209.png)
  ...
  + HMR的原理
    ...
    + HMR Socket Server，是一个socket的长连接：
      ...
      + ![image-20230416125033531](.\img\image-20230416125033531.png)
...
+ compress是否为静态文件开启gzip compression：
  + 默认值是false，可以设置为true；
  + ![image-20230416130933923](.\img\image-20230416130933923.png)
...
+ 我们可以进行如下的设置：
  ...
  + changeOrigin：它表示是否更新代理后请求的headers中host地址；其实是要修改代理请求中的headers中的host属性：
    ...
    + ![image-20230416131250582](.\img\image-20230416131250582.png)
...
+ 另一个非常好用的功能是配置别名alias：
  ...
  + 我们可以给某些常见的路径起一个别名；
  + ![image-20230416132104079](.\img\image-20230416132104079.png)
...
+ ![image-20230416134144347](.\img\image-20230416134144347.png)
...
+ **入口文件的解析**
  ...
  + context的作用是用于解析入口（entry point）和加载器（loader）：
    ...
    + 另外推荐在配置中传入一个值；
    + <img src=".\img\image-20230416134432144.png" alt="image-20230416134432144" style="zoom:50%;" /><img src=".\img\image-20230416134436843.png" alt="image-20230416134436843" style="zoom:50%;" />
...
+ 通过Vue的命令来创建项目
  + `Vue create 项目的名称`
+ ![image-20230416134759735](.\img\image-20230416134759735.png)
+ ![image-20230416134804138](.\img\image-20230416134804138.png)
+ ![image-20230416134809324](.\img\image-20230416134809324.png)

![image-20230416134814191](.\img\image-20230416134814191.png)

**项目的目录结构**

![image-20230416134835249](.\img\image-20230416134835249.png)



![image-20230416134840424](.\img\image-20230416134840424.png)

**运行原理**

​	![image-20230416134900914](.\img\image-20230416134900914.png)
...
![image-20230416140538021](.\img\image-20230416140538021.png)
...
+ vite直接支持postcss的转换：
  ...
  + ![image-20230416140945475](.\img\image-20230416140945475.png)
...
+ ![image-20230416141240662](.\img\image-20230416141240662.png)

#### 3.8vite 打包项目

​	以直接通过vite build来完成对当前项目的打包工具：

![image-20230416141318701](.\img\image-20230416141318701.png)
...
![image-20230416141501471](.\img\image-20230416141501471.png)
